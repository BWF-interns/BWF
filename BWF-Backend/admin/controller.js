...
const LivePost       = require('../warden/models/livePost');
...
exports.deactivateStaff = async (req, res) => {
  try {
    const before = await StaffMember.findById(req.params.id).lean();
    if (!before) return res.status(404).json({ message: 'Staff not found' });
    const updated = await StaffMember.findByIdAndUpdate(req.params.id, { status:'inactive', leftOn: new Date() }, { new: true });
...
exports.upsertFinanceKPI = async (req, res) => {
  try {
    ...
    const kpi = await FinanceKPI.findOneAndUpdate({ year, month, home }, req.body, { new: true, upsert: true, setDefaultsOnInsert: true });
...
exports.reviewFeedback = async (req, res) => {
  try {
    ...
    const updated = await Feedback.findByIdAndUpdate(req.params.id,
      { status, reviewNote, reviewedBy: req.user.auth_id }, { new: true });
...
exports.updateGrievance = async (req, res) => {
  try {
    ...
    const updated = await Grievance.findByIdAndUpdate(req.params.id, updates, { new: true });
    if (!updated) return res.status(404).json({ message: 'Not found' });
    res.json(updated);
  } catch (err) { res.status(400).json({ message: err.message }); }
};
...
exports.listPendingPosts = async (req, res) => {
  try {
    ...
    res.json(await PendingPost.find(filter).sort({ createdAt: -1 }));
  } catch (err) { res.status(500).json({ message: err.message }); }
};
...
exports.reviewPendingPost = async (req, res) => {
  try {
    ...
    const pending = await PendingPost.findById(req.params.id);
    ...
    // If approved, copy to live Post collection
    if (status === 'approved') {
      await LivePost.create({
        content: pending.content,
        type:    pending.type,
        tags:    pending.tags,
        pollOptions: pending.pollOptions.map(o => ({ text: o.text, votes: 0 })),
        creatorId:   pending.creatorId,
        creatorName: pending.creatorName,
        creatorRole: pending.creatorRole,
        hostelName:  pending.hostelName,
        approvedBy:  req.user.auth_id,
        originalPendingId: String(pending._id),
      });
    }
...
exports.deletePendingPost = async (req, res) => {
  try {
    const p = await PendingPost.findByIdAndDelete(req.params.id);
...
exports.listLivePosts = async (req, res) => {
  try {
    ...
    if (type)       filter.type = type;
    // Pinned posts first
    const posts = await LivePost.find(filter).sort({ pinned: -1, createdAt: -1 });
    res.json(posts);
  } catch (err) { res.status(500).json({ message: err.message }); }
};
...
exports.updateLivePost = async (req, res) => {
  try {
    ...
    const updated = await LivePost.findByIdAndUpdate(req.params.id, req.body, { new: true });
