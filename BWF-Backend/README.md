...
MONGO_LOCAL_URI=mongodb://127.0.0.1:27017/bwf_local
...
- `MONGO_URI` points to MongoDB Atlas for shared usage.
- If Atlas is not ready, set `PREFER_LOCAL_DB=true` to work locally.
- Later you can upload local data to Atlas using `mongodump` + `mongorestore`.
- Keep old backups by storing dumps in a dated archive folder (for example: `backup-archive/2026-04-02`).
